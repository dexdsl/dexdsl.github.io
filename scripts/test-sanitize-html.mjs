...
const headerSlotCount = (sanitized.match(/\/assets\/js\/header-slot\.js(?:\?v=[^"'<>\\s]+)?/g) || []).length;
const grainOverlayCount = (sanitized.match(/\/assets\/js\/dx-grain-overlay\.js(?:\?v=[^"'<>\\s]+)?/g) || []).length;
...
assert.equal($('#scroll-gradient-bg').length, 0, 'Entry documents should not own the persistent gradient layer');
assert.equal($('#gooey-mesh-wrapper').length, 0, 'Entry documents should not own the persistent mesh layer');
assert.equal($('style#dex-entry-gooey-bg-style').length, 0, 'Entry documents should not include legacy mesh styles');
assert.equal($('script#dex-entry-gooey-bg-script').length, 0, 'Entry documents should not include a route-local mesh runtime');
