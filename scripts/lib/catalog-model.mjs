...
export function buildSearchIndex(model) {
  ...
  return {
    ...
    entries: entries.map((entry) => ({
      ...
      site_norm: normalizeSearch(entry.uav?.site?.name),
      subject_norm: normalizeSearch((entry.uav?.subjects || []).map((row) => row.label).join(' ')),
      uav: entry.uav,
      ...
      search_blob: normalizeSearch([
        ...
        (entry.uav?.subjects || []).map((row) => row.label).join(' '),
        (entry.uav?.capture_classes || []).join(' '),
        (entry.uav?.spectra || []).join(' '),
      ].join(' ')),
    })),
  };
}
